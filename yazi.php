...
if ($_GET['page']) {
    ...
    $sorgu = mysqli_query($conn, "SELECT * FROM sayfalar WHERE id='{$_GET['page']}'");
    $bilgi = mysqli_fetch_array($sorgu);
    mysqli_query($conn, "UPDATE sayfalar SET hit=hit+1 WHERE id='{$_GET['page']}'");
    echo "<ul>";
    echo "<i class=\"fa fa-user-circle-o\" title=\"Yazar\"><a href=\"#\">{$bilgi['konu_bolumu']}</a></i>";
    echo "<br/><br/>";
